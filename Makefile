...
qemu: all
	qemu-system-arm -kernel ./build/$(APP_NAME).elf -machine lm3s6965evb -vnc :0 -serial mon:stdio

qemu-uart: all
	qemu-system-arm -kernel ./build/$(APP_NAME).elf -machine lm3s6965evb -nographic

qemu-gdb: all
	qemu-system-arm -kernel ./build/$(APP_NAME).elf -S -s -machine lm3s6965evb -vnc :0 -serial mon:stdio
