...
int main(void) {
	...
	xTaskCreate(prvTask1, "T1", configMINIMAL_STACK_SIZE + 50, (void*) &task1,
	...
	xTaskCreate(prvTask5, "T5", configMINIMAL_STACK_SIZE + 50, (void*) &task5,
...
void prvTask(void *pvParameters) {
	...
	for (;;) {
		// Formatea el mensaje para incluir el nombre de la tarea, el contador de lanzamientos y el tiempo en ticks
		sprintf(cMessage, "S %s - Count: %u - Time: %u ticks\n\r",
		pcTaskGetTaskName( NULL), uxReleaseCount, xTaskGetTickCount());

		prvPrintString(cMessage);

		vBusyWait(task->wcet);

		sprintf(cMessage, "E %s - Count: %u - Time: %u ticks\n\r",
		pcTaskGetTaskName( NULL), uxReleaseCount, xTaskGetTickCount());
...
void prvTask5(void *pvParameters) {
	char cMessage[mainMAX_MSG_LEN];
	unsigned int uxReleaseCount = 0;
	TickType_t pxPreviousWakeTime = 0;
	xTask *task = (xTask*) pvParameters;
